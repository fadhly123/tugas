...
$password = md5($password);
 // Ambil value password yang dikirim dari form
//$pa$password = md5($password)ssword = md5($password); // Kita enkripsi (encrypt) password tadi dengan md5

// Buat query untuk mengecek apakah ada data user dengan username dan password yang dikirim dari form
$sql = mysqli_query($conn, "SELECT * FROM tb_akun WHERE username='".$username."' AND password='".$password."'");
